  ...
  return (
    <section>
      ...
      <p className="text-center text-slate-500 max-w-2xl mx-auto mb-8">
        Temukan inspirasi resep minuman menyegarkan
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {recipes.map((recipe, index) => {
          const isMinuman = recipe.kategori === "minuman";

          return (
            <div
              key={`${recipe.id}-${recipe.kategori}`}
            ...
            >
              <Link to={`/resep/${recipe.kategori}/${recipe.id}`}>
                <div
                  className={`relative backdrop-blur-xl border border-white/25 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg md:shadow-2xl transition-all duration-500 cursor-pointer group-hover:scale-105 group-hover:bg-white/20 ${
                    isMinuman
                      ? "bg-white/15 shadow-blue-500/5 hover:shadow-blue-500/15"
                      : "bg-white/15 shadow-green-500/5 hover:shadow-green-500/15"
                  }`}
                >
                  {/* Efek hover */}
